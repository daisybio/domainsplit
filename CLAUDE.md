...
- One SwissProt flat file feeds **five** consumers: GO terms, the accession→Pfam map, the protein FASTA, the STRING-id map (`DR STRING;`, which replaced the per-organism `url_uniprot_id_mapping` download) and `FETCH_DOMAIN_META`'s protein universe. HIPPIE is read by **UniProt accession only**; a file without `uniprot_accession_A/B` columns is rejected rather than silently yielding zero DDIs.
...
- **`instance_tier` and `swissprot_taxon_ids` are one setting in two params.** `swissprot_taxon_ids` decides which proteins exist (it is the filter `PARSE_SWISSPROT` applies and the universe `FETCH_DOMAIN_META` samples from); `instance_tier` decides which strata of that universe are eligible. `any` against a taxon-filtered universe is a no-op that looks like it worked, so `workflows/domainsplit.nf` refuses both mismatched combinations before any task is submitted. `ingest_instances.py` asserts the same invariant at the other end.
- **Domain instances come from `Pfam-A.regions.tsv.gz`, never `Pfam-A.fasta`.** Pfam publishes the FASTA 90 % non-redundant, which drops the human member of every well-conserved family — a 12,769-family human request kept only 2,140. The regions file has no redundancy reduction; it carries coordinates only, so the Swiss-Prot flat file supplies sequence (`SQ`) and taxon (`OX`) and, being Swiss-Prot, the reviewed set. That fix lives in the submodule's `bin/fetch_domains.py`.
