...
- `domain.id` is a surrogate integer. The published embedding HDF5s key on it, so a file is valid across every split DB **of the same run** and silently wrong across runs; the `domainsplit_run` root attribute exists to detect that.
