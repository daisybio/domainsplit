...
1. **Shared trunk (all 4 lines stacked / running parallel through the same stations).** Stations in order, left to right:
   ...
   - `GENERATE_ESM_PROTEIN_EMBEDDINGS` **and** `GENERATE_ESM_DOMAIN_EMBEDDINGS` — render these as two stations on a short parallel pair (like an interchange) since the ESM module emits both. Both rejoin the trunk immediately after.
   - `COBINET` — interchange / hub station (slightly larger dot or square), marks assembly of `cobinet.sqlite3`.
...
- Add a small subtitle: "Pipeline: daisybio/domainsplit — CoBiNet DB assembly + train/optimization/test splitting".
- Add a tiny annotation near `COBINET` ("assembles cobinet.sqlite3") and near `SPLIT_DATABASE` ("emits per-method, per-split SQLite DBs").

### What NOT to include

- No external input URLs as stations (3did, UniProt, Negatome, STRING, pfam2go are inputs to `COBINET` but should not appear on the map).
...
**What:** Queries the 3did SQLite for every Pfam ID referenced by a known DDI. Defines the universe of domains the pipeline will care about downstream.
...
**What:** Joins UniProt accessions to Pfam domain occurrences using the alignments. Produces the master mapping of which proteins contain which Pfam domains — the backbone of CoBiNet's protein/domain bridge.
...
### `COBINET` (interchange / hub)
**In:** `3did.sqlite3`, Negatome, pfam2go, UniProt sequences, protein↔domain mapping, ProtT5 embeddings, UniProt GO terms, STRING links, UniProt id-mapping, ESM protein embeddings, ESM domain embeddings.
**Out:** Single unified `cobinet.sqlite3` ("the CoBiNet DB").
**What:** The assembly step. Merges every data source into one SQLite database with tables for DDIs, PPIs (positive + negative), GO annotations, sequences, and both embedding modalities. This is the canonical artifact the splitting half of the pipeline consumes.

### `EXTRACT_PROTEIN_SEQUENCES` / `EXTRACT_DOMAIN_SEQUENCES`
**In:** `cobinet.sqlite3`.
**Out:** Two FASTA files — `proteins.fasta` and `domains.fasta`.
**What:** Pulls the protein and domain sequences out of the CoBiNet DB into FASTA so they can be clustered. Domain FASTAs are sub-sequences carved using the Pfam coordinates from `CREATE_PROTEIN_DOMAIN_MAPPING`.
...
**In:** `cobinet.sqlite3` + split ratios `[train 0.6, optimization 0.2, test 0.2]`.
...
**In:** `cobinet.sqlite3`, denoise fraction `0.1`, discovery flag `false`.
...
**In:** `cobinet.sqlite3`, denoise fraction `0.1`, discovery flag `true`.
**Out:** Per-split DDI ID files.
...
**In:** `cobinet.sqlite3`, split ratios, and the corresponding cluster TSV (protein or domain) from MMseqs2.
...
**In:** All per-method, per-split DDI ID files (random_ddi, random_denoise, random_discovery, minimal_leakage_protein, minimal_leakage_domain) joined with their split labels and the full `cobinet.sqlite3`.
